...
#             masked_data = np.ma.masked_array(np_vars, np.isnan(np_vars))
            
#             if np.any(np.isnan(np_vars)):
#                 self.logger.info("aggregating is nan at round %s", current_round)
# #             np_vars = list(np.nan_to_num(np_vars, nan = 0.0))
            new_val = np.sum(np_vars, axis=0) / len(self.accumulator)
#             new_val = np.ma.average(masked_data, axis=0, weights=[1]*len(self.accumulator))
