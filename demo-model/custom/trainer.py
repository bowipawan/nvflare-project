...
class SimpleTrainer(Trainer):
    ...
    def setup(self, fl_ctx):
        ...
        if client_name == "org1":
            ...
            self.test_images = self.test_images[:len(self.test_images)//2-1000]
            self.test_labels = self.test_labels[:len(self.test_labels)//2-1000] 
        elif client_name == "org2":
            self.train_images = self.train_images[len(self.train_images)//2:]
            self.train_labels = self.train_labels[len(self.train_labels)//2:]
            self.test_images = self.test_images[len(self.test_images)//2 - 1000:len(self.test_labels)- 2000]
            self.test_labels = self.test_labels[len(self.test_labels)//2 - 1000:len(self.test_labels)- 2000] 
    ...
    def train(self, shareable: Shareable, fl_ctx: FLContext, abort_signal) -> Shareable:
        ...
        self.model.save_weights("/workspace/nvflare/model-test")
